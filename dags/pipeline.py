...
from src.spark.spark_connect import sc  # Import biến sc như yêu cầu
...
def stock_full_etl():
    ...
    def crawl_stock_price(tickers):
        stock_price = VnstockDataCrawler(max_workers=2).fetch_price_history(tickers, '2024-01-01',
                                                                            '2025-12-31')
        bm.save_dataframe(df=stock_price, bucket=BUCKET_NAME, folder="market/prices", file_name="stock_price",
                          file_format='csv')

    def get_writer():
        return SparkWritePostgres(sc.spark, db_config)
    ...
    def process_financials_and_prices():
        ...
        writer.write_data(df_dim_date, "dim_date", mode="overwrite")  # Dim date có thể overwrite để tránh duplicate
